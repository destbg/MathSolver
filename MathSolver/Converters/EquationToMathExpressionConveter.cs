...
internal class EquationToMathExpressionConveter
{
    ...
    public MathExpression Convert()
    {
        ...
        if (equalConditionIndex != -1)
        {
            ...
            ConditionMathExpression expression = new(equalConditionType == ConditionType.Equal, leftCheck, rightCheck, ifTrue, ifFalse);
    ...
    private int IndexOfImportantSymbol()
    {
        for (int i = 0; i < expressions.Count; i++)
        {
            ...
            if (expression.Type == EquationType.Symbol)
            {
                ...
                if (symbol is not MathSymbol.Addition and not MathSymbol.Subraction)
                {
                    return i;
                }
            }
    ...
    private (ConditionType, int) IndexOfEqualCondition()
    {
        for (int i = 0; i < expressions.Count; i++)
        {
            ...
            if (expression.Type == EquationType.Condition)
            {
                ...
                if (condition is ConditionType.Equal or ConditionType.NotEqual)
                {
                    return (condition, i);
                }
            }
    ...
    private void SimplifyExpression(int expressionIndex)
    {
        ...
        MathExpression mathExpression = new UnaryMathExpression(leftExpression, rightExpression, symbol, BracketType.None, []);
